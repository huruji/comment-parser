...
PARSERS.parse_tag = function parse_tag(str, data, offset) {
  debugger;
  ...
  return {
    source: result[ 0 ],
    data: { tag: result[ 1 ], tagStart: str.indexOf(result[ 1 ]) + (offset || 0) }
  };
};

PARSERS.parse_type = function parse_type(str, data, offset) {
  ...
  return {
    ...
    data: { type: res.slice(1, -1), typeStart: str.indexOf(res) + 1 + (offset || 0) }
  };
};

PARSERS.parse_name = function parse_name(str, data, offset) {
  if (data.errors && data.errors.length) { return null; }
  ...
  res.name = name;
  res.nameStart = str.indexOf(name) + (offset || 0);
...
PARSERS.parse_description = function parse_description(str, data, offset) {
  if (data.errors && data.errors.length) { return null; }
  ...
  if (result) {
    return {
      source: result[ 0 ],
      data: { description: result[ 1 ] === undefined ? '' : result[ 1 ], descriptionStart: str.indexOf(result[ 1 ] || '') + (offset || 0) }
    };
  }
